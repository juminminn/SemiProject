...
@WebServlet("/mypage/overlap/nick")
public class MypageNickOverlapController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		System.out.println("------ [MypageNickOverlapController] Do Get ------");
		...
		//이메일 중복값 체크하는 메소드 
		// 2면 기존닉네임이랑 동일 1 이면 존재 0이면 없음
		check = mypageService.checkOverlapNick(req, member);
	
//		req.setAttribute("data", check); // 확인된 값 파라미터로 다시 전달
		req.setAttribute("check", check); // 확인된 값 파라미터로 다시 전달
		req.getRequestDispatcher("/WEB-INF/views/mypage/overlap/overlapNick.jsp")
		   .forward(req, resp);
	}
